# UBOTZ 2 Payment Feature Documentation

## 1. Scope
This document defines the tenant-side Payment feature from the current backend implementation, including routing surface, core modules, and tenant-isolation expectations.

## 2. Business Objectives
- Enable tenant administrators to operate payment workflows from the Tenant Admin Dashboard.
- Keep all records tenant-scoped and role/capability-protected.
- Provide stable APIs for frontend modules and reporting/status visibility.

## 3. Backend Implementation Footprint
- Application layer files: 11
- Domain layer files: 18
- Infrastructure persistence files: 3
- HTTP/controller files: 4

Primary module roots:
- backend/app/Application/TenantAdminDashboard/Payment
- backend/app/Domain/TenantAdminDashboard/Payment
- backend/app/Infrastructure/Persistence/TenantAdminDashboard/Payment

## 4. API Surface (Tenant Dashboard)
Route definition files:
- backend/routes/tenant_dashboard/payment.php
- backend/routes/tenant_dashboard/student_payments.php
- backend/routes/tenant_dashboard/billing.php

Representative endpoints:
- Route::post('/checkout/course', [CheckoutController::class, 'initializeCourseCheckout']);
- Route::get('/{id}', [StudentInvoiceReadController::class, 'show']);
- Route::get('/{id}/download', [StudentInvoiceReadController::class, 'download']);
- Route::post('/webhooks/payment', [CheckoutController::class, 'handleWebhook']);
- Route::post('/course-purchase/initiate', [StudentCoursePurchaseController::class, 'initiate']);
- Route::post('/course-purchase/verify', [StudentCoursePurchaseController::class, 'verify']);
- Route::post('/installment-step/initiate', [StudentInstallmentStepPurchaseController::class, 'initiate']);
- Route::post('/installment-step/verify', [StudentInstallmentStepPurchaseController::class, 'verify']);
- Route::get('/summary', [StudentFeeReadController::class, 'summary']);
- Route::get('/installments', [StudentFeeReadController::class, 'installments']);

## 5. Security and Tenant Isolation Requirements
- All queries and mutations must execute inside resolved tenant context (TenantContext).
- Endpoints should enforce capability middleware where required (for example tenant.capability:*).
- Responses must not leak cross-tenant entities through joins, eager loads, or error payloads.
- Audit-sensitive actions (status changes, destructive updates, impersonation-type flows) should remain logged.

## 6. Frontend Contract Notes
- Frontend pages should treat empty datasets as a normal state, not an error state.
- Validation failures should surface backend field messages directly.
- Lifecycle/status values should be normalized at UI boundaries where legacy values exist.

## 7. Status Tracking Reference
- documentation/Tenant feature/status reports/Payment_Status_Report.md

## 8. Next Documentation Steps
To match the Platform documentation standard, create alongside this file:
1. Payment_business_findings.md
2. Payment_technical_documentation.md
